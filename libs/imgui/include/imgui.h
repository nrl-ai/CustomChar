...
typedef int ImGuiKeyChord;        // -> ImGuiKey | ImGuiMod_XXX    // Flags: for
                            // storage only for now: an ImGuiKey optionally
...
IMGUI_API void PushTabStop(
    bool tab_stop);  // == tab stop enable. Allow focusing using TAB/Shift-TAB,
                     // enabled by default but you can disable it for certain
                     // widgets
...
IMGUI_API ImVec2
GetCursorScreenPos();  // cursor position in absolute coordinates (useful to
                       // work with ImDrawList API). generally top-left ==
...
enum ImGuiTreeNodeFlags_ {
  ...
  ImGuiTreeNodeFlags_SpanAvailWidth =
      1 << 11,  // Extend hit box to the right-most edge, even if not framed.
                ...
                // on the same line. In the future we may refactor the hit
                // system to be front-to-back, allowing natural overlaps and
...
enum ImGuiTabItemFlags_ {
  ...
  ImGuiTabItemFlags_UnsavedDocument =
      1 << 0,  // Display a dot next to the title + tab is selected when
               // clicking the X + closure is not assumed (will wait for user to
               // stop submitting the tab). Otherwise closure is assumed when
               // pressing the X, so if you keep submitting the tab may reappear
               // at end of tab bar.
...
struct ImGuiStyle {
  ...
  float TabMinWidthForCloseButton;  // Minimum width for close button to appear
                                    // on an unselected tab when hovered. Set to
                                    // 0.0f to always show when hovering, set to
                                    // FLT_MAX to never show close button unless
                                    // selected.
  ImGuiDir
...
struct ImGuiIO {
  ...
  bool ConfigWindowsMoveFromTitleBarOnly;  // = false       // Enable allowing
                                           // to move windows only when clicking
                                           // on their title bar. Does not apply
                                           // to windows without a title bar.
  float
  ...
  IMGUI_API void ClearInputKeys();    // Clear current keyboard/mouse/gamepad
                                    // state + current frame text input buffer.
  ...
  int KeyMap[ImGuiKey_COUNT];  // [LEGACY] Input: map of indices into the
                               ...
                               // your "native" keyboard state. The first 512
                               // are now unused and should be kept zero. Legacy
...
struct ImGuiTableColumnSortSpecs {
  ...
  ImGuiSortDirection
      SortDirection : 8;  // ImGuiSortDirection_Ascending or
                          ...
                          // SortSign, whichever is more convenient for your
                          // sort function)
...
struct ImGuiListClipper {
  ...
  IMGUI_API void IncludeRangeByIndices(
...
struct ImFontAtlas {
  ...
  int TexGlyphPadding;  // Padding between glyphs within texture in pixels.
                        ...
                        // on bilinear filtering you may set this to 0 (will
                        // also need to set AntiAliasedLinesUseTex = false).
  ...
  ImVector<ImFontAtlasCustomRect> CustomRects;  // Rectangles for packing custom
                                                // texture data into the atlas.
  ...
  unsigned int
      FontBuilderFlags;  // Shared flags (for all fonts) for custom font
                         // builder. THIS IS BUILD IMPLEMENTATION DEPENDENT.
                         // Per-font override is also available in ImFontConfig.
...
IMGUI_API bool ImageButton(
    ...
    const ImVec4& tint_col =
        ImVec4(1, 1, 1, 1));  // Use new ImageButton() signature (explicit item
...
enum ImDrawCornerFlags_ {
  ...
  ImDrawCornerFlags_BotLeft =
      ImDrawFlags_RoundCornersBottomLeft,  // Was == 0x04 (1 << 2) prior
                                           // to 1.82.
...
typedef ImGuiKeyChord
    ImGuiModFlags;  // == int. We generally use ImGuiKeyChord to mean "a
                    // ImGuiKey or-ed with any number of ImGuiMod_XXX value",
