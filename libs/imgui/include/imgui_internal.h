...
// we don't provide any guarantee of forward compatibility. To implement maths
// operators for ImVec2 (disabled by default to not conflict with using
...
//#define ImFloorStd(X)     floorf(X)           // We use our own, see ImFloor()
// and ImFloorSigned()
...
enum ImGuiItemStatusFlags_ {
  ...
  ImGuiItemStatusFlags_FocusedByTabbing =
      1 << 8,  // Set when the Focusable item just got focused by Tabbing
               // (FIXME: to be removed soon)
  ImGuiItemStatusFlags_Visible =
      1 << 9,  // [WIP] Set when item is overlapping the current clipping
...
struct ImGuiPopupData {
  ...
  ImGuiWindow* BackupNavWindow;  // Set on OpenPopup(), a NavWindow that will be
...
struct ImGuiNextItemData {
  ...
  ImGuiID FocusScopeId;  // Set by SetNextItemMultiSelectData() (!= 0 signify
                         ...
                         // HasSelectionData, we don't use Flags for this
                         // because they are cleared too early. This is mostly
                         // used for debugging)
  ImGuiCond OpenCond;
...
// SetKeyOwner(), SetItemKeyOwner() Don't mistake with ImGuiInputTextFlags! (for
// ImGui::InputText() function)
enum ImGuiInputFlags_ {
  // Flags for IsKeyPressed(), IsMouseClicked(), Shortcut()
  ImGuiInputFlags_None = 0,
  ImGuiInputFlags_Repeat = 1
                           << 0,  // Return true on successive repeats. Default
  ...
  ImGuiInputFlags_LockThisFrame =
      1 << 6,  // Access to key data will require EXPLICIT owner ID
               ...
               // at end of frame. This is useful to make input-owner-aware code
  ...
  // - Using ImGuiInputFlags_RouteAlways is roughly equivalent to doing e.g.
  ...
  ImGuiInputFlags_RouteFocused =
      1 << 8,  // (Default) Register focused route: Accept inputs if window is
               // in focus stack. Deep-most focused window takes inputs.
               // ActiveId takes inputs over deep-most focused window.
...
struct ImGuiViewportP : public ImGuiViewport {
  ...
  ImDrawList* DrawLists[2];   // Convenience background (0) and foreground (1)
                              ...
                              // cursor when io.MouseDrawCursor is set and to
                              // draw most debug overlays.
  ImDrawData DrawDataP;
  ImDrawDataBuilder
      DrawDataBuilder;   // Temporary data while building final ImDrawData
  ImVec2 WorkOffsetMin;  // Work Area: Offset from Pos to top-left corner of
                         // Work Area. Generally (0,0) or
                         // (0,+main_menu_bar_height). Work Area is Full Area
                         // but without menu-bars/status-bars (so WorkArea
                         // always fit inside Pos/Size!)
  ImVec2 WorkOffsetMax;  // Work Area: Offset from Pos+Size to bottom-right
                         // corner of Work Area. Generally (0,0) or
                         // (0,-status_bar_height).
...
struct ImGuiContext {
  ...
  ImGuiID DebugHookIdInfo;  // Will call core hooks: DebugHookIdInfo() from
                            // GetID functions, used by Stack Tool [next
  ...
  ImU32
      ActiveIdUsingNavInputMask;  // If you used this. Since (IMGUI_VERSION_NUM
  ...
  bool
      NavDisableHighlight;  // When user starts using mouse, we hide
  ...
  float ScrollbarClickDeltaToGrabCenter;  // Distance between mouse and center
...
struct ImGuiTableInstanceData {
  ...
  float LastFirstRowHeight;  // Height of first row from last frame (FIXME: this
                             // is used as "header height" and may be reworked)
...
// useful for debugging/metrics. sizeof() ~ 112 bytes.
...
IMGUI_API bool
IsItemToggledSelection();  // Was the last item selection toggled? (after
                           // Selectable(), TreeNode() etc. We only returns
                           // toggle _event_ in order to handle clipping
                           // correctly)
...
IMGUI_API void GetKeyChordName(ImGuiKeyChord key_chord, char* out_buf,
                               int out_buf_size);
...
//  ImGuiItemStatusFlags_FocusedTabbing) != 0 || (g.NavActivateId == id &&
//  (g.NavActivateFlags & ImGuiActivateFlags_PreferInput))' (WIP)
