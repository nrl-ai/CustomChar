...
struct RenderResources {
  ...
  WGPUBindGroup ImageBindGroup =
      nullptr;  // Default font-resource of Dear ImGui
...
static void SafeRelease(WGPUTextureView& res) {
  if (res) wgpuTextureViewRelease(res);
  res = nullptr;
}
static void SafeRelease(WGPUTexture& res) {
  if (res) wgpuTextureRelease(res);
  res = nullptr;
}
...
void ImGui_ImplWGPU_RenderDrawData(ImDrawData* draw_data,
  ...
  if (fr->VertexBuffer == nullptr ||
    ...
    WGPUBufferDescriptor vb_desc = {
        ...
        MEMALIGN(fr->VertexBufferSize * sizeof(ImDrawVert), 4), false};
  ...
  if (fr->IndexBuffer == nullptr ||
    ...
    WGPUBufferDescriptor ib_desc = {
        ...
        MEMALIGN(fr->IndexBufferSize * sizeof(ImDrawIdx), 4), false};
...
static void ImGui_ImplWGPU_CreateFontsTexture() {
  ...
  static_assert(
      sizeof(ImTextureID) >= sizeof(bd->renderResources.FontTexture),
...
static void ImGui_ImplWGPU_CreateUniformBuffer() {
  ...
  WGPUBufferDescriptor ub_desc = {
      nullptr, "Dear ImGui Uniform buffer",
...
bool ImGui_ImplWGPU_CreateDeviceObjects() {
  ...
  bd->renderResources.ImageBindGroups.SetVoidPtr(
      ImHashData(&bd->renderResources.FontTextureView, sizeof(ImTextureID)),
      image_bind_group);
...
void ImGui_ImplWGPU_NewFrame() {
  ImGui_ImplWGPU_Data* bd = ImGui_ImplWGPU_GetBackendData();
  if (!bd->pipelineState) ImGui_ImplWGPU_CreateDeviceObjects();
}
