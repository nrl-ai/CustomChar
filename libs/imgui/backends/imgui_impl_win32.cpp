...
//  2023-04-19: Added ImGui_ImplWin32_InitForOpenGL() to facilitate combining
  ...
  switch (msg) {
      ...
      if (wParam < 256) {
        ...
        if (vk == VK_SHIFT) {
          ...
          if (IsVkDown(VK_RMENU) == is_key_down) {
            ImGui_ImplWin32_AddKeyEvent(ImGuiKey_RightAlt, is_key_down,
                                        VK_RMENU, scancode);
          }
        }
      }
    ...
    case WM_CHAR:
      if (::IsWindowUnicode(hwnd)) {
        ...
        wchar_t wch = 0;
        ::MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, (char*)&wParam, 1, &wch,
                              1);
