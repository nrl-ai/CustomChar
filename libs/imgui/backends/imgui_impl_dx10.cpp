...
void ImGui_ImplDX10_RenderDrawData(ImDrawData* draw_data) {
  ...
  if (!bd->pVB || bd->VertexBufferSize < draw_data->TotalVtxCount) {
    ...
    desc.MiscFlags = 0;
    if (ctx->CreateBuffer(&desc, nullptr, &bd->pVB) < 0) return;
  }
  ...
  if (!bd->pIB || bd->IndexBufferSize < draw_data->TotalIdxCount) {
    ...
    desc.CPUAccessFlags = D3D10_CPU_ACCESS_WRITE;
    if (ctx->CreateBuffer(&desc, nullptr, &bd->pIB) < 0) return;
  }
...
bool ImGui_ImplDX10_CreateDeviceObjects() {
  ...
  if (bd->pFontSampler) ImGui_ImplDX10_InvalidateDeviceObjects();
  ...
  // Create the vertex shader
  {
    ...
    D3D10_INPUT_ELEMENT_DESC local_layout[] = {
        {"POSITION", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
         (UINT)IM_OFFSETOF(ImDrawVert, pos), D3D10_INPUT_PER_VERTEX_DATA, 0},
        {"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
         (UINT)IM_OFFSETOF(ImDrawVert, uv), D3D10_INPUT_PER_VERTEX_DATA, 0},
        {"COLOR", 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0,
         (UINT)IM_OFFSETOF(ImDrawVert, col), D3D10_INPUT_PER_VERTEX_DATA, 0},
    };
...
void ImGui_ImplDX10_NewFrame() {
  ...
  if (!bd->pFontSampler) ImGui_ImplDX10_CreateDeviceObjects();
}
