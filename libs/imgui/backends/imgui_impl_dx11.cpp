...
//  support (64k+ vertices) with 16-bit indices.
...
//  bd->XXXX). 2021-05-19: DirectX11: Replaced direct access to
...
//  bound when calling ImGui_ImplDX10_RenderDrawData. Clearing
...
static void ImGui_ImplDX11_SetupRenderState(ImDrawData* draw_data,
  ...
  ctx->HSSetShader(nullptr, nullptr,
                   0);  // In theory we should backup and restore this as well..
...
void ImGui_ImplDX11_RenderDrawData(ImDrawData* draw_data) {
  ...
  // (0,0) for single viewport apps.
  {
    ...
    if (ctx->Map(bd->pVertexConstantBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0,
...
bool ImGui_ImplDX11_CreateDeviceObjects() {
  ...
  if (bd->pFontSampler) ImGui_ImplDX11_InvalidateDeviceObjects();
  ...
  // Create the vertex shader
  {
    ...
    D3D11_INPUT_ELEMENT_DESC local_layout[] = {
        {"POSITION", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
         (UINT)IM_OFFSETOF(ImDrawVert, pos), D3D11_INPUT_PER_VERTEX_DATA, 0},
        {"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
         (UINT)IM_OFFSETOF(ImDrawVert, uv), D3D11_INPUT_PER_VERTEX_DATA, 0},
        {"COLOR", 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0,
         (UINT)IM_OFFSETOF(ImDrawVert, col), D3D11_INPUT_PER_VERTEX_DATA, 0},
    };
...
void ImGui_ImplDX11_InvalidateDeviceObjects() {
  ...
  }  // We copied data->pFontTextureView to io.Fonts->TexID so let's clear that
     // as well.
  if (bd->pIB) {
    bd->pIB->Release();
    bd->pIB = nullptr;
...
void ImGui_ImplDX11_NewFrame() {
  ...
  if (!bd->pFontSampler) ImGui_ImplDX11_CreateDeviceObjects();
}
