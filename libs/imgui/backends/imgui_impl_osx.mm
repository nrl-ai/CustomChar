...
//  ImGuiConfigFlags_NoMouseCursorChange'. [X] Platform: Mouse support. Can discriminate Mouse/Pen.
...
//  values will also be supported unless IMGUI_DISABLE_OBSOLETE_KEYIO is set] [X] Platform: OSX
//  clipboard is supported within core Dear ImGui (no specific code in this backend). [X] Platform:
...
//  2023-04-09: Inputs: Added support for io.AddMouseSourceEvent() to discriminate
//  ImGuiMouseSource_Mouse/ImGuiMouseSource_Pen. 2023-02-01: Fixed scroll wheel scaling for devices
//  emitting events with hasPreciseScrollingDeltas==false (e.g. non-Apple mices). 2022-11-02: Fixed
//  mouse coordinates before clicking the host window. 2022-10-06: Fixed mouse inputs on flipped
//  views. 2022-09-26: Inputs: Renamed ImGuiKey_ModXXX introduced in 1.87 to ImGuiMod_XXX (old names
//  still supported). 2022-05-03: Inputs: Removed ImGui_ImplOSX_HandleEvent() from backend API in
//  favor of backend automatically handling event capture. 2022-04-27: Misc: Store backend data in a
//  per-context struct, allowing to use this backend with multiple contexts. 2022-03-22: Inputs:
//  Monitor NSKeyUp events to catch missing keyUp for key when user press Cmd + key 2022-02-07:
//  Inputs: Forward keyDown/keyUp events to OS when unused by dear imgui. 2022-01-31: Fixed building
//  with old Xcode versions that are missing gamepad features. 2022-01-26: Inputs: replaced
...
//  io.AddMouseWheelEvent() API (1.87+). 2022-01-12: Inputs: Added basic Platform IME support,
//  hooking the io.SetPlatformImeDataFn() function. 2022-01-10: Inputs: calling new
//  io.AddKeyEvent(), io.AddKeyModsEvent() + io.SetKeyEventNativeData() API (1.87+). Support for
//  full ImGuiKey range. 2021-12-13: *BREAKING CHANGE* Add NSView parameter to ImGui_ImplOSX_Init().
//  Generally fix keyboard support. Using kVK_* codes for keyboard keys. 2021-12-13: Add game
//  controller support. 2021-09-21: Use mach_absolute_time as CFAbsoluteTimeGetCurrent can jump
//  backwards. 2021-08-17: Calling io.AddFocusEvent() on
//  NSApplicationDidBecomeActiveNotification/NSApplicationDidResignActiveNotification events.
//  2021-06-23: Inputs: Added a fix for shortcuts using CTRL key instead of CMD key.
//  2021-04-19: Inputs: Added a fix for keys remaining stuck in pressed state when CMD-tabbing into
//  different application. 2021-01-27: Inputs: Added a fix for mouse position not being reported
//  when mouse buttons other than left one are down. 2020-10-28: Inputs: Added a fix for handling
//  keypad-enter key. 2020-05-25: Inputs: Added a fix for missing trackpad clicks when done with
//  "soft tap". 2019-12-05: Inputs: Added support for ImGuiMouseCursor_NotAllowed mouse cursor.
//  2019-10-11: Inputs:  Fix using Backspace key.
//  2019-07-21: Re-added clipboard handlers as they are not enabled by default in core imgui.cpp
//  (reverted 2019-05-18 change). 2019-05-28: Inputs: Added mouse cursor shape and visibility
//  support. 2019-05-18: Misc: Removed clipboard handlers as they are now supported by core
//  imgui.cpp. 2019-05-11: Inputs: Don't filter character values before calling AddInputCharacter()
//  apart from 0xF700..0xFFFF range. 2018-11-30: Misc: Setting up io.BackendPlatformName so it can
//  be displayed in the About Window. 2018-07-07: Initial version.

#define APPLE_HAS_BUTTON_OPTIONS                                                              \
  (__IPHONE_OS_VERSION_MIN_REQUIRED >= 130000 || __MAC_OS_X_VERSION_MIN_REQUIRED >= 101500 || \
   __TV_OS_VERSION_MIN_REQUIRED >= 130000)
#define APPLE_HAS_CONTROLLER                                                                  \
  (__IPHONE_OS_VERSION_MIN_REQUIRED >= 140000 || __MAC_OS_X_VERSION_MIN_REQUIRED >= 110000 || \
   __TV_OS_VERSION_MIN_REQUIRED >= 140000)
#define APPLE_HAS_THUMBSTICKS                                                                 \
  (__IPHONE_OS_VERSION_MIN_REQUIRED >= 120100 || __MAC_OS_X_VERSION_MIN_REQUIRED >= 101401 || \
...
static ImGui_ImplOSX_Data* ImGui_ImplOSX_CreateBackendData() {
  return IM_NEW(ImGui_ImplOSX_Data)();
}
static ImGui_ImplOSX_Data* ImGui_ImplOSX_GetBackendData() {
  return (ImGui_ImplOSX_Data*)ImGui::GetIO().BackendPlatformUserData;
}
...
/**
 KeyEventResponder implements the NSTextInputClient protocol as is required by the macOS text input
 manager.

 The macOS text input manager is invoked by calling the interpretKeyEvents method from the keyDown
 method. Keyboard events are then evaluated by the macOS input manager and valid text input is
 passed back via the insertText:replacementRange method.
...
- (void)keyDown:(NSEvent*)event {
  if (!ImGui_ImplOSX_HandleEvent(event, self)) [super keyDown:event];
...
- (void)keyUp:(NSEvent*)event {
  if (!ImGui_ImplOSX_HandleEvent(event, self)) [super keyUp:event];
}
...
bool ImGui_ImplOSX_Init(NSView* view) {
  ...
  // io.BackendFlags |= ImGuiBackendFlags_HasSetMousePos;          // We can honor
  // io.WantSetMousePos requests (optional, rarely used)
  ...
  bd->MouseCursors[ImGuiMouseCursor_ResizeNESW] =
      [NSCursor respondsToSelector:@selector(_windowResizeNorthEastSouthWestCursor)]
  ...
  // '-framework ApplicationServices' to your linker command-line. Since we are already in ObjC land
  // here, it is easy for us to add a clipboard handler using the NSPasteboard api.
...
static void ImGui_ImplOSX_UpdateImePosWithView(NSView* view) {
  ImGui_ImplOSX_Data* bd = ImGui_ImplOSX_GetBackendData();
  ImGuiIO& io = ImGui::GetIO();
  if (io.WantTextInput) [bd->KeyEventResponder updateImePosWithView:view];
}
...
void ImGui_ImplOSX_NewFrame(NSView* view) {
  ...
  // Setup time step
  if (bd->Time == 0.0) bd->Time = GetMachAbsoluteTimeInSeconds();
...
// (Note that NSEventTypeScrollWheel is considered "other input". Oddly enough an exception does not
// occur with it, but the value will sometimes be wrong!)
static ImGuiMouseSource GetMouseSource(NSEvent* event) {
  switch (event.subtype) {
    case NSEventSubtypeTabletPoint:
      return ImGuiMouseSource_Pen;
    // macOS considers input from relative touch devices (like the trackpad or Apple Magic Mouse) to
    // be touch input. This doesn't really make sense for Dear ImGui, which expects absolute touch
    // devices only. There does not seem to be a simple way to disambiguate things here so we
    // consider NSEventSubtypeTouch events to always come from mice. See
...
static bool ImGui_ImplOSX_HandleEvent(NSEvent* event, NSView* view) {
  ...
  if (event.type == NSEventTypeMouseMoved || event.type == NSEventTypeLeftMouseDragged ||
    ...
    if (event.window == nil) mousePoint = [[view window] convertPointFromScreen:mousePoint];
  ...
  if (event.type == NSEventTypeScrollWheel) {
    ...
    // here.
    if (event.phase == NSEventPhaseCancelled) return false;
  ...
  if (event.type == NSEventTypeKeyDown || event.type == NSEventTypeKeyUp) {
    if ([event isARepeat]) return io.WantCaptureKeyboard;
...
static void ImGui_ImplOSX_AddTrackingArea(NSView* _Nonnull view) {
  ...
  // we receive events for all controls, not just Dear ImGui widgets. If we had native controls in
  // our window, we'd want to be much more careful than just ingesting the complete event stream. To
  // match the behavior of other backends, we pass every event down to the OS.
  ImGui_ImplOSX_Data* bd = ImGui_ImplOSX_GetBackendData();
  if (bd->Monitor) return;
