    ...
    if (kid >= 0) {                                                            \
      ...
      if (ktype != (type)) {                                                   \
        throw std::runtime_error(format("key %s has wrong type: %s",           \
                                        skey.c_str(), gguf_type_name(ktype))); \
      }                                                                        \
      (dst) = func(ctx, kid);                                                  \
    } else if (req) {                                                          \
...
static void ggml_offload_nop(struct ggml_tensor* tensor) { (void)tensor; }

static std::string llama_token_to_piece(const struct llama_context* ctx,
                                        llama_token token) {
...
struct llama_hparams {
  ...
  bool operator!=(const llama_hparams& other) const {
    ...
    if (!is_float_close(this->f_norm_rms_eps, other.f_norm_rms_eps, EPSILON))
      return true;
    if (!is_float_close(this->rope_freq_base_train, other.rope_freq_base_train,
                        EPSILON))
      return true;
    if (!is_float_close(this->rope_freq_scale_train,
                        other.rope_freq_scale_train, EPSILON))
      return true;
...
static bool llama_kv_cache_init(const struct llama_hparams& hparams,
  ...
  cache.buf.resize(2u * n_elements * ggml_type_size(wtype) +
                   2u * ggml_tensor_overhead());
...
static void llama_kv_cache_seq_rm(struct llama_kv_cache& cache,
...
static void llama_kv_cache_seq_cp(struct llama_kv_cache& cache,
...
static void llama_kv_cache_seq_keep(struct llama_kv_cache& cache,
                                    llama_seq_id seq_id) {
  uint32_t new_head = cache.size;
...
static void llama_kv_cache_seq_shift(struct llama_kv_cache& cache,
                                     llama_seq_id seq_id, llama_pos p0,
  ...
  for (uint32_t i = 0; i < cache.size; ++i) {
    if (cache.cells[i].has_seq_id(seq_id) && cache.cells[i].pos >= p0 &&
        cache.cells[i].pos < p1) {
...
struct llama_model_loader {
  ...
  struct ggml_tensor* create_tensor(struct ggml_context* ctx,
    ...
    if (backend == GGML_BACKEND_GPU_SPLIT) {
      if (ne.size() == 1) {
        throw std::runtime_error(
            format("%s: 1-dimensional tensor '%s' cannot be split on the GPU",
                   __func__, name.c_str()));
      }
    }

    {
      ...
      if (!is_ok) {
        throw std::runtime_error(format(
            "%s: tensor '%s' has wrong shape; expected %s, got %s", __func__,
  ...
  void done_getting_tensors() const {
    if (n_created != n_tensors) {
      throw std::runtime_error(
          format("%s: wrong number of tensors; expected %d, got %d", __func__,
                 n_tensors, n_created));
    }
  }
  ...
  void load_all_data(struct ggml_context* ctx,
    ...
    for (int i = 0; i < gguf_get_n_tensors(ctx_gguf); i++) {
      struct ggml_tensor* cur =
          ggml_get_tensor(ctx, gguf_get_tensor_name(ctx_gguf, i));
      GGML_ASSERT(
          cur);  // unused tensors should have been caught by load_data already
...
static void llm_load_hparams(llama_model_loader& ml, llama_model& model) {
  ...
  GGUF_GET_KEY(ctx, hparams.rope_finetuned, gguf_get_val_bool, GGUF_TYPE_BOOL,
               false, kv(LLM_KV_ROPE_SCALING_FINETUNED));

  hparams.n_yarn_orig_ctx = hparams.n_ctx_train;
  GGUF_GET_KEY(ctx, hparams.n_yarn_orig_ctx, gguf_get_val_u32, GGUF_TYPE_UINT32,
               false, kv(LLM_KV_ROPE_SCALING_ORIG_CTX_LEN));

  // rope_freq_base (optional)
  hparams.rope_freq_base_train = 10000.0f;
  GGUF_GET_KEY(ctx, hparams.rope_freq_base_train, gguf_get_val_f32,
               GGUF_TYPE_FLOAT32, false, kv(LLM_KV_ROPE_FREQ_BASE));

  std::string rope_scaling("linear");
  GGUF_GET_KEY(ctx, rope_scaling, gguf_get_val_str, GGUF_TYPE_STRING, false,
               kv(LLM_KV_ROPE_SCALING_TYPE));
  hparams.rope_scaling_type_train =
      llama_rope_scaling_type_from_string(rope_scaling);
  ...
  // sanity check for n_rot (optional)
  {
    ...
    if (model.arch == LLM_ARCH_LLAMA || model.arch == LLM_ARCH_FALCON) {
      if (hparams.n_rot != hparams.n_embd / hparams.n_head) {
        throw std::runtime_error(format("invalid n_rot: %u, expected %u",
                                        hparams.n_rot,
  ...
  switch (model.arch) {
    ...
    case LLM_ARCH_FALCON: {
      GGUF_GET_KEY(ctx, hparams.f_norm_eps, gguf_get_val_f32, GGUF_TYPE_FLOAT32,
                   true, kv(LLM_KV_ATTENTION_LAYERNORM_EPS));
    ...
    case LLM_ARCH_MPT: {
      ...
      GGUF_GET_KEY(ctx, hparams.f_norm_eps, gguf_get_val_f32, GGUF_TYPE_FLOAT32,
                   true, kv(LLM_KV_ATTENTION_LAYERNORM_EPS));
      GGUF_GET_KEY(ctx, hparams.f_clamp_kqv, gguf_get_val_f32,
                   GGUF_TYPE_FLOAT32, false, kv(LLM_KV_ATTENTION_CLAMP_KQV));
      GGUF_GET_KEY(ctx, hparams.f_max_alibi_bias, gguf_get_val_f32,
                   GGUF_TYPE_FLOAT32, true,
...
static std::vector<llama_vocab::id> llama_tokenize_internal(
    const llama_vocab& vocab, std::string raw_text, bool bos,
    bool special = false);
...
static void llm_load_vocab(llama_model_loader& ml, llama_model& model) {
  ...
  for (uint32_t i = 0; i < n_vocab; i++) {
    ...
    token_data.type =
        toktypes ? (llama_token_type)toktypes[i] : LLAMA_TOKEN_TYPE_NORMAL;
  }
  ...
  if (vocab.type == LLAMA_VOCAB_TYPE_SPM) {
    ...
    const std::vector<int> ids =
        llama_tokenize_internal(vocab, "\u010A", false);
  ...
  {
    // TODO: It is unclear (to me) at this point, whether special tokes are
    // guaranteed to be of a deterministic type,
    //  and will always be correctly labeled in 'added_tokens.json' etc.
    // The assumption is, since special tokens aren't meant to be exposed to end
    // user, they are designed
    //  to be unmatchable by the tokenizer, therefore tokens from the vocab,
    //  which are unmatchable by the tokenizer are special tokens.
    ...
    for (const auto& t : vocab.token_to_id) {
      ...
      if (token.length() > 1) {
        ...
        if (!is_tokenizable) {
          // Some tokens are multibyte, but they are utf sequences with
          // equivalent text length of 1
          //  it's faster to re-filter them here, since there are way less
          //  candidates now
          ...
          if (utf8_str_len > 1) {
            ...
            // If this manually found special token is not marked as such, flag
            // a mismatch
            if (vocab.id_to_token[id].type == LLAMA_TOKEN_TYPE_NORMAL) {
    ...
    if (special_tokens_definition_mismatch ||
      ...
      LLAMA_LOG_WARN(
          ...
          __func__, special_tokens_count_from_verification,
          vocab.id_to_token.size(), special_tokens_count_by_type,
...
static void llm_load_print_meta(llama_model_loader& ml, llama_model& model) {
  ...
  const auto rope_scaling_type =
      LLAMA_ROPE_SCALING_TYPES.at(hparams.rope_scaling_type_train);
  ...
  LLAMA_LOG_INFO("%s: rope_finetuned   = %s\n", __func__,
                 hparams.rope_finetuned ? "yes" : "unknown");
  ...
  size_t vram_weights = 0;
  {
    ...
    switch (model.arch) {
      ...
      case LLM_ARCH_REFACT: {
        ...
        // output
        {
          ...
          if (n_gpu_layers > int(n_layer)) {
            // norm is not performance relevant on its own but keeping it in
            // VRAM reduces data copying on Windows however this is detrimental
            // unless everything is on the GPU
#ifndef _WIN32
            backend_norm = LLAMA_BACKEND_OFFLOAD;
#else
            backend_norm = n_gpu_layers <= (int)n_layer + 2
                               ? GGML_BACKEND_CPU
        ...
        for (uint32_t i = 0; i < n_layer; ++i) {
          const ggml_backend_type backend =
              int(i) < i_gpu_start ? GGML_BACKEND_CPU
                                   : LLAMA_BACKEND_OFFLOAD;  // NOLINT
          const ggml_backend_type backend_split =
              int(i) < i_gpu_start ? GGML_BACKEND_CPU
          ...
          layer.attn_norm = ml.create_tensor(
              ctx, tn(LLM_TENSOR_ATTN_NORM, "weight", i), {n_embd}, backend);
          ...
          layer.ffn_norm = ml.create_tensor(
              ctx, tn(LLM_TENSOR_FFN_NORM, "weight", i), {n_embd}, backend);
          ...
          if (backend == GGML_BACKEND_GPU) {
            vram_weights +=
                ...
                ggml_nbytes(layer.ffn_gate) + ggml_nbytes(layer.ffn_down) +
      ...
      case LLM_ARCH_FALCON: {
        ...
        // output
        {
          ...
          model.output_norm_b = ml.create_tensor(
              ctx, tn(LLM_TENSOR_OUTPUT_NORM, "bias"), {n_embd}, backend_norm);
          model.output = ml.create_tensor(ctx, tn(LLM_TENSOR_OUTPUT, "weight"),
                                          {n_embd, n_vocab}, backend_output);
        ...
        for (uint32_t i = 0; i < n_layer; ++i) {
          ...
          layer.attn_norm_b = ml.create_tensor(
              ctx, tn(LLM_TENSOR_ATTN_NORM, "bias", i), {n_embd}, backend);

          if (gguf_find_tensor(
                  ml.ctx_gguf,
                  tn(LLM_TENSOR_ATTN_NORM_2, "weight", i).c_str()) >= 0) {
            layer.attn_norm_2 =
                ml.create_tensor(ctx, tn(LLM_TENSOR_ATTN_NORM_2, "weight", i),
                                 {n_embd}, backend);
      ...
      case LLM_ARCH_STARCODER: {
        model.tok_embd =
            ml.create_tensor(ctx, tn(LLM_TENSOR_TOKEN_EMBD, "weight"),
                             {n_embd, n_vocab}, GGML_BACKEND_CPU);
        model.pos_embd =
            ml.create_tensor(ctx, tn(LLM_TENSOR_POS_EMBD, "weight"),
                             {n_embd, hparams.n_ctx_train}, GGML_BACKEND_CPU);
        ...
        for (uint32_t i = 0; i < n_layer; ++i) {
          ...
          layer.wqkv = ml.create_tensor(
              ctx, tn(LLM_TENSOR_ATTN_QKV, "weight", i),
              {n_embd, n_embd + 2 * n_embd_gqa}, backend_split);
          layer.bqkv = ml.create_tensor(ctx, tn(LLM_TENSOR_ATTN_QKV, "bias", i),
                                        {n_embd + 2 * n_embd_gqa}, backend);

          layer.wo = ml.create_tensor(ctx, tn(LLM_TENSOR_ATTN_OUT, "weight", i),
                                      {n_embd, n_embd}, backend_split);
          layer.bo = ml.create_tensor(ctx, tn(LLM_TENSOR_ATTN_OUT, "bias", i),
                                      {n_embd}, backend);

          layer.ffn_norm = ml.create_tensor(
              ctx, tn(LLM_TENSOR_FFN_NORM, "weight", i), {n_embd}, backend);
          layer.ffn_norm_b = ml.create_tensor(
              ctx, tn(LLM_TENSOR_FFN_NORM, "bias", i), {n_embd}, backend);

          layer.ffn_down =
              ml.create_tensor(ctx, tn(LLM_TENSOR_FFN_DOWN, "weight", i),
                               {n_ff, n_embd}, backend_split);
          layer.ffn_down_b = ml.create_tensor(
              ctx, tn(LLM_TENSOR_FFN_DOWN, "bias", i), {n_embd}, backend);

          layer.ffn_up =
              ml.create_tensor(ctx, tn(LLM_TENSOR_FFN_UP, "weight", i),
                               {n_embd, n_ff}, backend_split);
          layer.ffn_up_b = ml.create_tensor(
              ctx, tn(LLM_TENSOR_FFN_UP, "bias", i), {n_ff}, backend);
      ...
      case LLM_ARCH_PERSIMMON: {
        ...
        for (uint32_t i = 0; i < n_layer; ++i) {
          const ggml_backend_type backend =
              int(i) < i_gpu_start ? GGML_BACKEND_CPU : LLAMA_BACKEND_OFFLOAD;
          const ggml_backend_type backend_split =
              int(i) < i_gpu_start ? GGML_BACKEND_CPU
          ...
          layer.attn_q_norm = ml.create_tensor(
              ctx, tn(LLM_TENSOR_ATTN_Q_NORM, "weight", i), {64}, backend);
          layer.attn_q_norm_b = ml.create_tensor(
              ctx, tn(LLM_TENSOR_ATTN_Q_NORM, "bias", i), {64}, backend);
          layer.attn_k_norm = ml.create_tensor(
              ctx, tn(LLM_TENSOR_ATTN_K_NORM, "weight", i), {64}, backend);
          layer.attn_k_norm_b = ml.create_tensor(
              ctx, tn(LLM_TENSOR_ATTN_K_NORM, "bias", i), {64}, backend);
        }
      } break;
      case LLM_ARCH_BLOOM: {
        // TODO: CPU-only for now
        ...
        model.tok_embd =
            ml.create_tensor(ctx, tn(LLM_TENSOR_TOKEN_EMBD, "weight"),
                             {n_embd, n_vocab}, GGML_BACKEND_CPU);
        model.tok_norm =
            ml.create_tensor(ctx, tn(LLM_TENSOR_TOKEN_EMBD_NORM, "weight"),
                             {n_embd}, GGML_BACKEND_CPU);
        model.tok_norm_b =
            ml.create_tensor(ctx, tn(LLM_TENSOR_TOKEN_EMBD_NORM, "bias"),
                             {n_embd}, GGML_BACKEND_CPU);
      ...
      case LLM_ARCH_MPT: {
        ...
        for (uint32_t i = 0; i < n_layer; ++i) {
          ...
          if (backend == GGML_BACKEND_GPU) {
            vram_weights +=
                ggml_nbytes(layer.attn_norm) + ggml_nbytes(layer.wqkv) +
                ggml_nbytes(layer.wo) + ggml_nbytes(layer.ffn_norm) +
                ggml_nbytes(layer.ffn_down) + ggml_nbytes(layer.ffn_up);
          }
        }
...
static bool llama_model_load(const std::string& fname, llama_model& model,
  ...
  try {
    ...
    llm_load_tensors(ml, model, params.n_gpu_layers, params.main_gpu,
...
using llm_build_cb =
    std::function<void(struct ggml_tensor* cur, const char* name, int nl)>;
...
static struct ggml_tensor* llm_build_inp_embd(struct ggml_context* ctx,
  ...
  if (batch.token) {
    struct ggml_tensor* inp_tokens =
        ggml_new_tensor_1d(ctx, GGML_TYPE_I32, batch.n_tokens);
...
static void llm_build_k_shift(struct ggml_context* ctx,
                              const llama_hparams& hparams,
                              const llama_cparams& cparams,
                              const llama_kv_cache& kv,
                              struct ggml_cgraph* graph, llm_rope_type type,
                              int64_t n_ctx, int64_t n_rot, float freq_base,
                              float freq_scale, const llm_build_cb& cb) {
  const int64_t n_layer = hparams.n_layer;
  ...
  for (int il = 0; il < n_layer; ++il) {
    struct ggml_tensor* tmp =
        ...
        ggml_rope_custom_inplace(
            ctx,
            ggml_view_3d(ctx, kv.k, n_rot, n_head_kv, n_ctx,
...
static void llm_build_kv_store(
    struct ggml_context* ctx, const llama_hparams& hparams,
    const llama_kv_cache& kv, struct ggml_cgraph* graph,
    struct ggml_tensor* k_cur, struct ggml_tensor* v_cur, int64_t n_ctx,
    int32_t n_tokens, int32_t kv_head, const llm_build_cb& cb, int64_t il) {
  const int64_t n_embd_gqa = hparams.n_embd_gqa();

  // compute the transposed [n_tokens, n_embd] V matrix
  struct ggml_tensor* v_cur_t =
      ggml_transpose(ctx, ggml_reshape_2d(ctx, v_cur, n_embd_gqa, n_tokens));
  ...
  struct ggml_tensor* k_cache_view = ggml_view_1d(
      ctx, kv.k, n_tokens * n_embd_gqa,
      (ggml_element_size(kv.k) * n_embd_gqa) * (il * n_ctx + kv_head));
  cb(k_cache_view, "k_cache_view", il);

  struct ggml_tensor* v_cache_view = ggml_view_2d(
      ctx, kv.v, n_tokens, n_embd_gqa, (n_ctx)*ggml_element_size(kv.v),
      (il * n_ctx) * ggml_element_size(kv.v) * n_embd_gqa +
          kv_head * ggml_element_size(kv.v));
...
static struct ggml_tensor* llm_build_norm(struct ggml_context* ctx,
                                          struct ggml_tensor* cur,
                                          const llama_hparams& hparams,
...
static struct ggml_tensor* llm_build_ffn(
    struct ggml_context* ctx, struct ggml_tensor* cur, struct ggml_tensor* up,
    struct ggml_tensor* up_b, struct ggml_tensor* gate,
    struct ggml_tensor* gate_b, struct ggml_tensor* down,
...
static struct ggml_tensor* llm_build_kqv(
    struct ggml_context* ctx, const llama_hparams& hparams,
    const llama_kv_cache& kv, struct ggml_tensor* wo, struct ggml_tensor* wo_b,
    struct ggml_tensor* q_cur, struct ggml_tensor* kq_scale,
    struct ggml_tensor* kq_mask, int64_t n_ctx, int32_t n_tokens, int32_t n_kv,
    float max_alibi_bias, const llm_build_cb& cb, int il) {
  const int64_t n_embd = hparams.n_embd;
  ...
  struct ggml_tensor* k =
      ggml_view_3d(ctx, kv.k, n_embd_head, n_kv, n_head_kv,
                   ggml_element_size(kv.k) * n_embd_gqa,
                   ggml_element_size(kv.k) * n_embd_head,
                   ggml_element_size(kv.k) * n_embd_gqa * n_ctx * il);
  cb(k, "k", il);
  ...
  struct ggml_tensor* v = ggml_view_3d(
      ctx, kv.v, n_kv, n_embd_head, n_head_kv, ggml_element_size(kv.v) * n_ctx,
...
struct llm_build_context {
  ...
  const int64_t
      n_ctx;  // user-specified context size (can be different from n_ctx_train)
  ...
  llm_build_context(llama_context& lctx, const llama_batch& batch,
                    const llm_build_cb& cb, bool worst_case)
      : model(lctx.model),
  ...
  struct ggml_cgraph* build_llama() {
    ...
    struct ggml_tensor* KQ_mask =
        ggml_new_tensor_3d(ctx0, GGML_TYPE_F32, n_kv, n_tokens, 1);
    cb(KQ_mask, "KQ_mask", -1);

    // shift the entire K-cache if needed
    if (do_rope_shift) {
      llm_build_k_shift(ctx0, hparams, cparams, kv_self, gf, LLM_ROPE, n_ctx,
                        n_embd_head, freq_base, freq_scale, cb);
    }
    ...
    for (int il = 0; il < n_layer; ++il) {
      ...
      // norm
      cur = llm_build_norm(ctx0, inpL, hparams, model.layers[il].attn_norm,
      ...
      // self-attention
      {
        ...
        Qcur = ggml_rope_custom(
            ctx0, ggml_reshape_3d(ctx0, Qcur, n_embd_head, n_head, n_tokens),
            inp_pos, n_embd_head, 0, 0, n_orig_ctx, freq_base, freq_scale,
            ext_factor, attn_factor, beta_fast, beta_slow);
        cb(Qcur, "Qcur", il);

        Kcur = ggml_rope_custom(
            ctx0, ggml_reshape_3d(ctx0, Kcur, n_embd_head, n_head_kv, n_tokens),
            inp_pos, n_embd_head, 0, 0, n_orig_ctx, freq_base, freq_scale,
            ext_factor, attn_factor, beta_fast, beta_slow);
        cb(Kcur, "Kcur", il);

        llm_build_kv_store(ctx0, hparams, kv_self, gf, Kcur, Vcur, n_ctx,
                           n_tokens, kv_head, cb, il);

        cur = llm_build_kqv(ctx0, hparams, kv_self, model.layers[il].wo, NULL,
                            Qcur, KQ_scale, KQ_mask, n_ctx, n_tokens, n_kv,
                            -1.0f, cb, il);
        cb(cur, "kqv_out", il);
      }
      ...
      // feed-forward network
      {
        ...
        cur = llm_build_ffn(ctx0, cur, model.layers[il].ffn_up, NULL,
                            model.layers[il].ffn_gate, NULL,
                            model.layers[il].ffn_down, NULL, LLM_FFN_SILU,
                            LLM_FFN_PAR, cb, il);
        cb(cur, "ffn_out", il);
      }
    ...
    cur = llm_build_norm(ctx0, cur, hparams, model.output_norm, NULL,
                         LLM_NORM_RMS, cb, -1);
      ...
      // self-attention
      {
        ...
        switch (model.type) {
          ...
          case MODEL_13B:
            ...
            Kcur =
                ggml_reshape_3d(ctx0, Kcur, n_embd / n_head, n_head, n_tokens);
            break;
          default:
            GGML_ASSERT(false);
        }
  ...
  struct ggml_cgraph* build_falcon() {
    ...
    // shift the entire K-cache if needed
    if (do_rope_shift) {
      llm_build_k_shift(ctx0, hparams, cparams, kv_self, gf, LLM_ROPE_NEOX,
                        n_ctx, n_embd_head, freq_base, freq_scale, cb);
    }
    ...
    for (int il = 0; il < n_layer; ++il) {
      ...
      attn_norm =
          llm_build_norm(ctx0, inpL, hparams, model.layers[il].attn_norm,
                         model.layers[il].attn_norm_b, LLM_NORM, cb, il);
      ...
      // self-attention
      {
        if (model.layers[il].attn_norm_2) {
          // Falcon-40B
          cur =
              llm_build_norm(ctx0, inpL, hparams, model.layers[il].attn_norm_2,
                             model.layers[il].attn_norm_2_b, LLM_NORM, cb, il);
        ...
        struct ggml_tensor* Qcur = ggml_cont(
            ctx0, ggml_view_2d(ctx0, cur, n_embd, n_tokens, cur->nb[1],
                               0 * sizeof(float) * (n_embd)));
        struct ggml_tensor* Kcur = ggml_cont(
            ctx0, ggml_view_2d(ctx0, cur, n_embd_gqa, n_tokens, cur->nb[1],
                               1 * sizeof(float) * (n_embd)));
        struct ggml_tensor* Vcur = ggml_cont(
            ctx0, ggml_view_2d(ctx0, cur, n_embd_gqa, n_tokens, cur->nb[1],
                               1 * sizeof(float) * (n_embd + n_embd_gqa)));
        ...
        Qcur = ggml_rope_custom(ctx0, Qcur, inp_pos, n_embd_head, 2, 0,
                                n_orig_ctx, freq_base, freq_scale, ext_factor,
                                attn_factor, beta_fast, beta_slow);
        cb(Qcur, "Qcur", il);

        Kcur = ggml_rope_custom(ctx0, Kcur, inp_pos, n_embd_head, 2, 0,
                                n_orig_ctx, freq_base, freq_scale, ext_factor,
                                attn_factor, beta_fast, beta_slow);
        cb(Kcur, "Kcur", il);
      ...
      {
        cur = llm_build_ffn(
            ctx0, attn_norm,  // !! use the attn norm, not the result
            model.layers[il].ffn_up, NULL, NULL, NULL,
            model.layers[il].ffn_down, NULL, LLM_FFN_GELU, LLM_FFN_SEQ, cb, il);
        cb(cur, "ffn_out", il);
      }
    ...
    cur = llm_build_norm(ctx0, cur, hparams, model.output_norm,
                         model.output_norm_b, LLM_NORM, cb, -1);
    cb(cur, "result_norm", -1);
  ...
  struct ggml_cgraph* build_starcoder() {
    ...
    for (int il = 0; il < n_layer; ++il) {
      ...
      // self-attention
      {
        ...
        cur = llm_build_kqv(ctx0, hparams, kv_self, model.layers[il].wo,
                            model.layers[il].bo, Qcur, KQ_scale, KQ_mask, n_ctx,
                            n_tokens, n_kv, -1.0f, cb, il);
        cb(cur, "kqv_out", il);
      }
      ...
      {
        cur = llm_build_norm(ctx0, ffn_inp, hparams, model.layers[il].ffn_norm,
                             model.layers[il].ffn_norm_b, LLM_NORM, cb, il);
        cb(cur, "ffn_norm", il);

        cur = llm_build_ffn(
            ctx0, cur, model.layers[il].ffn_up, model.layers[il].ffn_up_b, NULL,
            NULL, model.layers[il].ffn_down, model.layers[il].ffn_down_b,
    ...
    cur = llm_build_norm(ctx0, inpL, hparams, model.output_norm,
                         model.output_norm_b, LLM_NORM, cb, -1);
    cb(cur, "result_norm", -1);
  ...
  struct ggml_cgraph* build_persimmon() {
    ...
    for (int il = 0; il < n_layer; ++il) {
      ...
      // self attention
      {
        ...
        struct ggml_tensor* tmpqkv =
            ggml_reshape_4d(ctx0, cur, n_embd_head, 3, n_head, n_tokens);
        cb(tmpqkv, "tmpqkv", il);

        struct ggml_tensor* tmpqkv_perm =
            ggml_cont(ctx0, ggml_permute(ctx0, tmpqkv, 0, 3, 1, 2));
        cb(tmpqkv_perm, "tmpqkv", il);

        struct ggml_tensor* tmpq = ggml_view_3d(
            ctx0, tmpqkv_perm, n_embd_head, n_head, n_tokens,
            ggml_element_size(tmpqkv_perm) * n_embd_head,
            ggml_element_size(tmpqkv_perm) * n_embd_head * n_head, 0);
        cb(tmpq, "tmpq", il);
        ...
        // Q/K Layernorm
        tmpq = llm_build_norm(ctx0, tmpq, hparams, model.layers[il].attn_q_norm,
                              model.layers[il].attn_q_norm_b, LLM_NORM, cb, il);
        cb(tmpq, "tmpq", il);

        tmpk = llm_build_norm(ctx0, tmpk, hparams, model.layers[il].attn_k_norm,
                              model.layers[il].attn_k_norm_b, LLM_NORM, cb, il);
        cb(tmpk, "tmpk", il);

        // RoPE the first n_rot of q/k, pass the other half, and concat.
        struct ggml_tensor* qrot =
            ggml_view_3d(ctx0, tmpq, n_rot, n_head, n_tokens,
                         ggml_element_size(tmpq) * n_embd_head,
                         ggml_element_size(tmpq) * n_embd_head * n_head, 0);
        cb(qrot, "qrot", il);

        struct ggml_tensor* krot =
            ggml_view_3d(ctx0, tmpk, n_rot, n_head, n_tokens,
                         ggml_element_size(tmpk) * n_embd_head,
                         ggml_element_size(tmpk) * n_embd_head * n_head, 0);
        cb(krot, "krot", il);

        // get the second half of tmpq, e.g tmpq[n_rot:, :, :]
        struct ggml_tensor* qpass =
            ggml_view_3d(ctx0, tmpq, n_rot, n_head, n_tokens,
                         ggml_element_size(tmpq) * n_embd_head,
        ...
        struct ggml_tensor* qrotated = ggml_rope_custom(
            ctx0, qrot, inp_pos, n_rot, 2, 0, n_orig_ctx, freq_base, freq_scale,
            ext_factor, attn_factor, beta_fast, beta_slow);
        cb(qrotated, "qrotated", il);

        struct ggml_tensor* krotated = ggml_rope_custom(
            ctx0, krot, inp_pos, n_rot, 2, 0, n_orig_ctx, freq_base, freq_scale,
            ext_factor, attn_factor, beta_fast, beta_slow);
        ...
        struct ggml_tensor* Q =
            ggml_cont(ctx0, ggml_permute(ctx0, Qcur, 1, 2, 0, 3));
        cb(Q, "Q", il);
        ...
        struct ggml_tensor* Vcur =
            ggml_view_3d(ctx0, tmpqkv_perm, n_embd_head, n_head, n_tokens,
                         ggml_element_size(tmpqkv_perm) * n_embd_head,
                         ggml_element_size(tmpqkv_perm) * n_embd_head * n_head,
                         ggml_element_size(tmpqkv_perm) * n_embd_head * n_head *
                             n_tokens * 2);
        ...
        cur = llm_build_kqv(ctx0, hparams, kv_self, model.layers[il].wo,
                            model.layers[il].bo, Q, KQ_scale, KQ_mask, n_ctx,
                            n_tokens, n_kv, -1.0f, cb, il);
        cb(cur, "kqv_out", il);
      }
  ...
  struct ggml_cgraph* build_bloom() {
    ...
    inpL = llm_build_norm(ctx0, inpL, hparams, model.tok_norm, model.tok_norm_b,
                          LLM_NORM, cb, -1);
    cb(inpL, "inp_norm", -1);
  ...
  struct ggml_cgraph* build_mpt() {
    ...
    for (int il = 0; il < n_layer; ++il) {
      ...
      // self-attention
      {
        ...
        if (hparams.f_clamp_kqv > 0.0f) {
          cur =
              ggml_clamp(ctx0, cur, -hparams.f_clamp_kqv, hparams.f_clamp_kqv);
        ...
        cur = llm_build_kqv(ctx0, hparams, kv_self, model.layers[il].wo, NULL,
                            Qcur, KQ_scale, KQ_mask, n_ctx, n_tokens, n_kv,
                            hparams.f_max_alibi_bias, cb, il);
        cb(cur, "kqv_out", il);
      }
      ...
      // feed forward
      {
        ...
        cur = llm_build_ffn(ctx0, cur, model.layers[il].ffn_up, NULL, NULL,
                            NULL, model.layers[il].ffn_down, NULL, LLM_FFN_GELU,
                            LLM_FFN_SEQ, cb, il);
        cb(cur, "ffn_out", il);
      }
    ...
    cur = llm_build_norm(ctx0, cur, hparams, model.output_norm, NULL, LLM_NORM,
                         cb, -1);
    cb(cur, "result_norm", -1);
...
struct llm_offload_trie {
  ...
  llm_offload_trie() { root = new node; }

  llm_offload_trie(
      const std::unordered_map<const char*, llm_offload_func_e>& map) {
    root = new node;
...
static const std::unordered_map<const char*, llm_offload_func_e> k_offload_map =
    {
        //{ "inp_tokens",                 OFFLOAD_FUNC_NR  }, // TODO: missing
        // K-quants get_rows kernel
        //{ "inp_embd",                   OFFLOAD_FUNC_NR  }, // TODO: missing
        // K-quants get_rows kernel
        {"pos_embd", OFFLOAD_FUNC_NR},

        {"inp_pos",
         OFFLOAD_FUNC_KQ},  // this is often used for KQ ops (e.g. rope)
...
#else
  const bool do_offload =
      true;  // TODO: set to false after finishing refactoring
#endif

  int n_non_view =
      0;  // number of non-view tensors that have been processed by the callback

  // this callback allows us to apply custom logic to each tensor (e.g.
  // ggml-alloc, offloading, etc.)
  // TODO: will be removed with backend v2
  llm_build_cb cb = [&](struct ggml_tensor* cur, const char* name, int il) {
    ...
    if (!alloc_inp_embd && strcmp(name, "inp_embd") == 0) {
      ...
      if (!ggml_allocr_is_measure(lctx.alloc) && batch.embd) {
        ...
        memcpy(cur->data, batch.embd,
               n_tokens * n_embd * ggml_element_size(cur));
      }
    ...
    static const std::unordered_map<llm_offload_func_e, std::string,
                                    std::hash<int>>
        k_offload_func_name = {
...
#else
            {OFFLOAD_FUNC, "CPU"},     {OFFLOAD_FUNC_KQ, "CPU"},
            {OFFLOAD_FUNC_V, "CPU"},   {OFFLOAD_FUNC_NR, "CPU"},
    ...
    if (func_e == OFFLOAD_FUNC_NOP) {
      ...
      if (worst_case) {
        LLAMA_LOG_WARN("%s: %32s: not offloaded (ref: %s)\n", __func__,
                       cur->name,
                       "https://github.com/ggerganov/llama.cpp/pull/3837");
      }
#endif
    ...
    if (worst_case) {
      LLAMA_LOG_INFO("%s: %32s: %s\n", __func__, cur->name,
                     k_offload_func_name.at(func_e).c_str());
    }
#endif
  };
  ...
  if (worst_case) {
    ...
    if (n_non_view != n_non_view_total) {
      LLAMA_LOG_WARN(
          ...
          __func__);
      LLAMA_LOG_WARN(
          "%s: not all non-view tensors have been processed with a callback\n",
          __func__);
      LLAMA_LOG_WARN(
          "%s: this can indicate an inefficiency in the graph implementation\n",
          __func__);
      LLAMA_LOG_WARN("%s: build with LLAMA_OFFLOAD_DEBUG for more info\n",
                     __func__);
      LLAMA_LOG_WARN(
          "%s: ref: https://github.com/ggerganov/llama.cpp/pull/3837\n",
          __func__);
      LLAMA_LOG_WARN(
          "%s: "
          "****************************************************************\n",
...
//
static int llama_decode_internal(llama_context& lctx, llama_batch batch) {
  const uint32_t n_tokens = batch.n_tokens;
  ...
  // if we start defragmenting the cache, the benefit from this will be more
  // important
  // kv_self.n = std::max(32, GGML_PAD(llama_kv_cache_cell_max(kv_self), 32));
  // // TODO: this might be better for CUDA?
  kv_self.n = std::min((int32_t)cparams.n_ctx,
                       std::max(32, llama_kv_cache_cell_max(kv_self)));
  ...
  // HACK: ggml-alloc may change the tensor backend when reusing a parent, so
  // force output to be on the CPU here if needed
  ...
  // If all tensors can be run on the GPU then using more than 1 thread is
  // detrimental.
  const bool full_offload_supported =
      model.arch == LLM_ARCH_LLAMA || model.arch == LLM_ARCH_BAICHUAN ||
      model.arch == LLM_ARCH_FALCON || model.arch == LLM_ARCH_REFACT ||
  ...
  //       need to update the graphs to skip "result_output"
  {
    ...
    if (batch.logits) {
      ...
      for (uint32_t i = 0; i < n_tokens; i++) {
        ...
        memcpy(logits_out.data() + (n_vocab * i),
               (float*)ggml_get_data(res) + (n_vocab * i),
               sizeof(float) * n_vocab);
      }
    } else if (lctx.logits_all) {
      logits_out.resize(n_vocab * n_tokens);
      memcpy(logits_out.data(), (float*)ggml_get_data(res),
             sizeof(float) * n_vocab * n_tokens);
    } else {
      logits_out.resize(n_vocab);
      memcpy(logits_out.data(),
             (float*)ggml_get_data(res) + (n_vocab * (n_tokens - 1)),
             sizeof(float) * n_vocab);
    }
  }
  ...
  if (!lctx.embedding.empty()) {
    ...
    memcpy(embedding_out.data(),
           (float*)ggml_get_data(embeddings) + (n_embd * (n_tokens - 1)),
           sizeof(float) * n_embd);
  }
...
static bool llama_is_user_defined_token(const llama_vocab& vocab,
                                        llama_token id) {
  ...
  void tokenize(const std::string& text, std::vector<llama_vocab::id>& output) {
    ...
    for (auto& word : word_collection) {
      ...
      while (offset < word.size()) {
        llm_symbol sym;
        size_t char_len =
            std::min(word.size() - offset, (size_t)::utf8_len(word[offset]));
 ...
 private:
  ...
  std::vector<std::string> bpe_gpt2_preprocess(const std::string& text) {
    ...
    for (int i = 0; i < (int)text_utf.size(); i++) {
      ...
      const std::string& utf_char_next =
          (i + 1 < (int)text_utf.size()) ? text_utf[i + 1] : "";
      const std::string& utf_char_next_next =
          (i + 2 < (int)text_utf.size()) ? text_utf[i + 2] : "";

      // handling contractions
      if (!split_condition && bytes_remain >= 2) {
        // 's|'t|'m|'d
        if (utf_char == "\'" &&
            (utf_char_next == "s" || utf_char_next == "t" ||
      ...
      if (!split_condition && !collecting) {
        if (codepoint_type(utf_char) == CODEPOINT_TYPE_LETTER ||
            (!token.size() && utf_char == " " &&
             codepoint_type(utf_char_next) == CODEPOINT_TYPE_LETTER)) {
          collecting_letter = true;
          collecting = true;
        } else if (codepoint_type(utf_char) == CODEPOINT_TYPE_DIGIT ||
                   (!token.size() && utf_char == " " &&
                    codepoint_type(utf_char_next) == CODEPOINT_TYPE_DIGIT)) {
          collecting_numeric = true;
          collecting = true;
        } else if (((codepoint_type(utf_char) != CODEPOINT_TYPE_LETTER &&
                     codepoint_type(utf_char) != CODEPOINT_TYPE_DIGIT) &&
                    (codepoint_type(utf_char) != CODEPOINT_TYPE_WHITESPACE)) ||
                   (!token.size() && utf_char == " " &&
                    codepoint_type(utf_char_next) != CODEPOINT_TYPE_LETTER &&
                    codepoint_type(utf_char_next) != CODEPOINT_TYPE_DIGIT &&
                    codepoint_type(utf_char_next) !=
                        CODEPOINT_TYPE_WHITESPACE)) {
          collecting_special = true;
          collecting = true;
        } else if (codepoint_type(utf_char) == CODEPOINT_TYPE_WHITESPACE &&
                   codepoint_type(utf_char_next) == CODEPOINT_TYPE_WHITESPACE) {
        ...
        if (collecting_letter &&
            codepoint_type(utf_char) != CODEPOINT_TYPE_LETTER) {
          split_condition = true;
...
struct fragment_buffer_variant {
  fragment_buffer_variant(llama_vocab::id _token)
      : type(FRAGMENT_BUFFER_VARIANT_TYPE_TOKEN),
        ...
        offset(0),
        length(0) {}
  fragment_buffer_variant(const std::string& _raw_text, int64_t _offset,
                          int64_t _length)
    ...
    while (it != buffer.end()) {
      ...
      if (fragment.type == FRAGMENT_BUFFER_VARIANT_TYPE_RAW_TEXT) {
        ...
        while (true) {
          ...
          //  passing offset argument only limit the "search area" but match
          //  coordinates are still relative to the source full raw_text
          auto match = raw_text->find(special_token, raw_text_base_offset);

          // no occurences found, stop processing this fragment for a given
          // special token
          ...
          fprintf(stderr, "FF: (%ld %ld %ld) '%s'\n", raw_text->length(),
                  raw_text_base_offset, raw_text_base_length,
                  raw_text->substr(raw_text_base_offset, raw_text_base_length)
                      .c_str());
#endif
          ...
          if (match > raw_text_base_offset) {
            ...
            buffer.emplace_after(it, (*raw_text), left_reminder_offset,
                                 left_reminder_length);

#ifdef PRETOKENIZERDEBUG
            fprintf(stderr, "FL: (%ld %ld) '%s'\n", left_reminder_offset,
                    left_reminder_length,
                    raw_text->substr(left_reminder_offset, left_reminder_length)
                        .c_str());
#endif
            it++;
          }
          ...
          if (match + special_token.length() <
              raw_text_base_offset + raw_text_base_length) {
            const int64_t right_reminder_offset =
                match + special_token.length();
            const int64_t right_reminder_length =
                raw_text_base_length -
                ((match - raw_text_base_offset) + special_token.length());
            buffer.emplace_after(it, (*raw_text), right_reminder_offset,
            ...
            fprintf(
                ...
                right_reminder_length,
                raw_text->substr(right_reminder_offset, right_reminder_length)
                    .c_str());
#endif
...
static std::vector<llama_vocab::id> llama_tokenize_internal(
    const llama_vocab& vocab, std::string raw_text, bool bos, bool special) {
    ...
    case LLAMA_VOCAB_TYPE_SPM: {
      for (const auto& fragment : fragment_buffer) {
        if (fragment.type == FRAGMENT_BUFFER_VARIANT_TYPE_RAW_TEXT) {
          ...
          //  by modifying llm_tokenizer_x to operate with string offsets like
          //  pre-tokenizer and passing 'add space prefix' as bool argument
          //
          auto raw_text =
              (special ? "" : " ") +
          ...
          fprintf(stderr, "TT: (%ld %ld %ld) '%s'\n", raw_text.length(),
                  fragment.offset, fragment.length, raw_text.c_str());
#endif
...
static std::pair<std::vector<uint32_t>, llama_partial_utf8> decode_utf8(
    const char* src, llama_partial_utf8 partial_start) {
...
static bool llama_grammar_match_partial_char(
    const llama_grammar_element* pos, const llama_partial_utf8 partial_utf8) {
  bool is_positive_char = pos->type == LLAMA_GRETYPE_CHAR;
...
void llama_sample_min_p(struct llama_context* ctx,
                        llama_token_data_array* candidates, float p,
                        size_t min_keep) {
...
void llama_sample_temp(struct llama_context* ctx,
                       llama_token_data_array* candidates_p, float temp) {
  const int64_t t_start_sample_us = ggml_time_us();
...
void llama_sample_repetition_penalties(struct llama_context* ctx,
  ...
  if (penalty_last_n == 0 || (penalty_repeat == 1.0f && penalty_freq == 0.0f &&
  ...
  for (size_t i = 0; i < candidates->size; ++i) {
    ...
    candidates->data[i].logit -=
        float(count) * penalty_freq + float(count > 0) * penalty_present;
  }
...
void llama_sample_grammar(struct llama_context* ctx,
  ...
  for (size_t i = 0; i < candidates->size; ++i) {
    ...
    if (id == eos) {
      ...
      candidates_grammar.push_back({i, candidates_decoded.back().first.data(),
...
struct llama_beam_search_data {
  ...
  //  * Gather elements until the vector is full, then call std::make_heap() on
  //  it.
  //  * If the heap is full and a new element is found that should be included,
  //  pop the
  //    least element to the back(), replace it with the new, then push it into
  //    the heap.
  void fill_next_beams_by_top_probabilities(llama_beam& beam) {
    ...
    if (beam.eob) {
      ...
      if (!beam.tokens.empty()) {
        llama_decode(ctx, llama_batch_get_one(beam.tokens.data(),
                                              beam.tokens.size(), n_past, 0));
      }
  ...
  void loop(const llama_beam_search_callback_fn_t callback,
    ...
    beams.push_back(
        {{},
         1.0f,
         false});  // Start with one empty beam w/ probability = 1.0 and !eob.
    const auto not_eob = [](const llama_beam& beam) { return !beam.eob; };
    for (int i = 0;
         i < n_predict && std::any_of(beams.begin(), beams.end(), not_eob) &&
         !beams[top_beam_index()].eob;
         ++i) {
      callback(callback_data,
               get_beams_state(false));  // Sets common_prefix_length
      update_beams_from_beam_views();    // Update values (p,eob) that callback
                                         // may have changed.
      if (common_prefix_length) {
        llama_decode(ctx, llama_batch_get_one(beams[0].tokens.data(),
                                              common_prefix_length, n_past, 0));
...
struct quantize_state_internal {
  ...
  quantize_state_internal(const llama_model& model,
                          const llama_model_quantize_params* params)
...
static ggml_type get_k_quant_type(quantize_state_internal& qs,
                                  ggml_type new_type, const ggml_tensor* tensor,
                                  llama_ftype ftype) {
  ...
  if (name == tn(LLM_TENSOR_OUTPUT, "weight")) {
    ...
    if (ftype == LLAMA_FTYPE_MOSTLY_Q2_K)
      ...
      new_type = GGML_TYPE_Q5_K;
             ...
             (qs.i_attention_wv < qs.n_attention_wv / 8 ||
              qs.i_attention_wv >= 7 * qs.n_attention_wv / 8))
      new_type = GGML_TYPE_Q6_K;
    if (qs.model.type == MODEL_70B) {
      // In the 70B model we have 8 heads sharing the same attn_v weights. As a
      // result, the attn_v.weight tensor is 8x smaller compared to
      // attn_q.weight. Hence, we can get a nice boost in quantization accuracy
      // with nearly negligible increase in model size by quantizing this tensor
      // with more bits:
    ...
    if (ftype == LLAMA_FTYPE_MOSTLY_Q2_K)
      ...
      new_type =
          qs.i_feed_forward_w2 < 2 ? GGML_TYPE_Q5_K
          : arch != LLM_ARCH_FALCON ||
                  use_more_bits(qs.i_feed_forward_w2, qs.n_feed_forward_w2)
              ? GGML_TYPE_Q4_K
              : GGML_TYPE_Q3_K;
    } else if (ftype == LLAMA_FTYPE_MOSTLY_Q3_K_L) {
      ...
      if (arch == LLM_ARCH_FALCON) {
        ...
        if (use_more_bits(qs.i_feed_forward_w2, qs.n_feed_forward_w2))
          new_type = GGML_TYPE_Q6_K;
      }
...
static void llama_model_quantize_internal(
  ...
  // mmap consistently increases speed Linux, and also increases speed on
  // Windows with hot cache. It may cause a slowdown on macOS, possibly related
  // to free memory.
  ...
  if (qs.n_attention_wv != qs.n_feed_forward_w2 ||
      (uint32_t)qs.n_attention_wv != model.hparams.n_layer) {
    LLAMA_LOG_WARN(
        "%s ============ Strange model: n_attention_wv = %d, n_feed_forward_w2 "
        "= %d, hparams.n_layer = %d\n",
        __func__, qs.n_attention_wv, qs.n_feed_forward_w2,
  ...
  for (int i = 0; i < ml.n_tensors; ++i) {
    ...
    if (!quantize) {
      ...
      if (tensor->type == GGML_TYPE_F32) {
        ...
        llama_convert_tensor_internal(tensor, f32_conv_buf, workers, nelements,
                                      nthread);
  ...
  if (qs.n_fallback > 0) {
    LLAMA_LOG_WARN(
        "%s: WARNING: %d of %d tensor(s) incompatible with k-quants and "
        "required fallback quantization\n",
        __func__, qs.n_fallback, qs.n_k_quantized + qs.n_fallback);
  }
}

static int llama_apply_lora_from_file_internal(const struct llama_model& model,
    ...
    if (lora_tensors.find(base_name + ".loraA") != lora_tensors.end() &&
      ...
      if (dest_t->backend == GGML_BACKEND_GPU ||
        ...
        if (dest_t->type != GGML_TYPE_F16) {
          throw std::runtime_error(format(
              "%s: error: the simultaneous use of LoRAs and GPU acceleration "
              "is only supported for f16 models. dest_t->type: %d",
              __func__, dest_t->type));
        }
      ...
      if (ggml_is_quantized(base_t->type)) {
        if (!warned) {
          LLAMA_LOG_WARN(
              "%s: warning: using a lora adapter with a quantized model may "
...
struct llama_context_params llama_context_default_params() {
  struct llama_context_params result = {
      ...
      /*.n_ctx                       =*/512,
      /*.n_batch                     =*/512,
      /*.n_threads                   =*/GGML_DEFAULT_N_THREADS,  // TODO: better
                                                                 // default
...
struct llama_context* llama_new_context_with_model(
  ...
  if (cparams.yarn_ext_factor < 0.0f) {  // negative indicates 'not set'
    cparams.yarn_ext_factor =
        rope_scaling_type == LLAMA_ROPE_SCALING_YARN ? 1.0f : 0.0f;
  }
  ...
  if (!hparams.vocab_only) {
    if (!llama_kv_cache_init(ctx->model.hparams, ctx->kv_self, memory_type,
                             cparams.n_ctx, model->n_gpu_layers)) {
      LLAMA_LOG_ERROR(
...

    {
      ...
      ctx->buf_compute.resize(ggml_tensor_overhead() * GGML_DEFAULT_GRAPH_SIZE +
                              ggml_graph_overhead());
      ...
      llama_token token = llama_token_bos(
          &ctx->model);  // not actually used by llama_build_graph, but required
                         // to choose between token and embedding inputs graph
      ggml_cgraph* gf = llama_build_graph(
          *ctx, llama_batch_get_one(&token, n_tokens, n_past, 0));
      ...
      if (model->n_gpu_layers > 0) {
        ...
        // ggml_allocr_set_parse_seq(ctx->alloc,
        // ggml_metal_get_concur_list(ctx->ctx_metal),
      ...
      ctx->alloc = ggml_allocr_new(ctx->buf_alloc.data, ctx->buf_alloc.size,
                                   tensor_alignment);
#ifdef GGML_USE_METAL
      if (ctx->ctx_metal) {
        // ggml_allocr_set_parse_seq(ctx->alloc,
      ...
      auto add_tensor = [](const ggml_tensor* t, size_t& size) {
        if (t->backend == GGML_BACKEND_GPU ||
            t->backend == GGML_BACKEND_GPU_SPLIT) {
          size += ggml_nbytes(t);
        }
      };
      ...
      LLAMA_LOG_INFO(
          "%s: total VRAM used: %.2f MB (model: %.2f MB, context: %.2f MB)\n",
          __func__, total_vram_size / 1024.0 / 1024.0,
          model_vram_size / 1024.0 / 1024.0, ctx_vram_size / 1024.0 / 1024.0);
#endif
    }
  ...
  if (!(result)) {                                           \
    ...
    return NULL;                                             \
      ...
      LLAMA_METAL_CHECK_BUF(ggml_metal_add_buffer(ctx->ctx_metal, "kv",
                                                  ctx->kv_self.buf.data,
                                                  ctx->kv_self.buf.size, 0));
      LLAMA_METAL_CHECK_BUF(ggml_metal_add_buffer(ctx->ctx_metal, "alloc",
                                                  ctx->buf_alloc.data,
...
#undef LLAMA_METAL_CHECK_BUF
  ...
  if (ggml_mpi_rank(ctx->ctx_mpi) > 0) {
    ...
    // llama_token_bos(ctx)); while (!llama_eval(ctx, tmp.data(), tmp.size(), 0,
    // 0)) {};
    llama_backend_free();
    exit(1);
  }
...
int llama_n_ctx(const struct llama_context* ctx) { return ctx->cparams.n_ctx; }
...
struct ggml_tensor* llama_get_model_tensor(struct llama_model* model,
                                           const char* name) {
...
int llama_apply_lora_from_file(struct llama_context* ctx, const char* path_lora,
                               float scale, const char* path_base_model,
                               int n_threads) {
  try {
    return llama_apply_lora_from_file_internal(ctx->model, path_lora, scale,
...
int llama_model_apply_lora_from_file(const struct llama_model* model,
                                     const char* path_lora, float scale,
                                     const char* path_base_model,
                                     int n_threads) {
  try {
    return llama_apply_lora_from_file_internal(*model, path_lora, scale,
                                               path_base_model, n_threads);
  } catch (const std::exception& err) {
    LLAMA_LOG_ERROR("%s: failed to apply lora adapter: %s\n", __func__,
                    err.what());
...
void llama_kv_cache_seq_rm(struct llama_context* ctx, llama_seq_id seq_id,
                           llama_pos p0, llama_pos p1) {
  llama_kv_cache_seq_rm(ctx->kv_self, seq_id, p0, p1);
}

void llama_kv_cache_seq_cp(struct llama_context* ctx, llama_seq_id seq_id_src,
                           llama_seq_id seq_id_dst, llama_pos p0,
                           llama_pos p1) {
...
void llama_kv_cache_seq_shift(struct llama_context* ctx, llama_seq_id seq_id,
 ...
 *
 */
static void llama_copy_state_data_internal(struct llama_context* ctx,
  ...
  // copy kv cache
  {
    ...
    if (kv_buf_size) {
      ...
      ggml_context* cpy_ctx =
          ggml_init({6 * ggml_tensor_overhead() + ggml_graph_overhead(), NULL,
                     /* no_alloc */ true});
      ...
      ggml_tensor* k3d =
          ggml_view_3d(cpy_ctx, kv_self.k, n_embd, kv_head, n_layer,
                       elt_size * n_embd, elt_size * n_embd * n_ctx, 0);

      ggml_tensor* v3d =
          ggml_view_3d(cpy_ctx, kv_self.v, kv_head, n_embd, n_layer,
...
size_t llama_set_state_data(struct llama_context* ctx, uint8_t* src) {
  ...
  // set kv cache
  {
    ...
    memcpy(&kv_buf_size, inp, sizeof(kv_buf_size));
    inp += sizeof(kv_buf_size);
    memcpy(&kv_head, inp, sizeof(kv_head));
    inp += sizeof(kv_head);
    ...
    if (kv_buf_size) {
      ...
      ggml_tensor* vin3d = ggml_new_tensor_3d(cpy_ctx, kv_self.v->type, kv_head,
                                              n_embd, n_layer);
      vin3d->data = (void*)inp;
    ...
    for (uint32_t i = 0; i < kv_size; ++i) {
      ...
      inp += sizeof(pos);
      memcpy(&seq_id_size, inp, sizeof(seq_id_size));
      inp += sizeof(seq_id_size);
      ...
      for (size_t j = 0; j < seq_id_size; ++j) {
        memcpy(&seq_id, inp, sizeof(seq_id));
        inp += sizeof(seq_id);
...
int llama_eval(struct llama_context* ctx, llama_token* tokens, int32_t n_tokens,
               int n_past) {
  llama_kv_cache_seq_rm(ctx->kv_self, -1, n_past, -1);

  const int ret = llama_decode_internal(
...
int llama_eval_embd(struct llama_context* ctx, float* embd, int32_t n_tokens,
                    int n_past) {
  llama_kv_cache_seq_rm(ctx->kv_self, -1, n_past, -1);

  llama_batch batch = {
      n_tokens, nullptr, embd, nullptr, nullptr, nullptr, nullptr, n_past, 1, 0,
  };
...
struct llama_batch llama_batch_get_one(llama_token* tokens, int32_t n_tokens,
                                       llama_pos pos_0, llama_seq_id seq_id) {
  return {
      /*n_tokens       =*/n_tokens,
      /*tokens         =*/tokens,
      /*embd           =*/nullptr,
...
const char* llama_token_get_text(const struct llama_model* model,
                                 llama_token token) {
  return model->vocab.id_to_token[token].text.c_str();
}

float llama_token_get_score(const struct llama_model* model,
                            llama_token token) {
  return model->vocab.id_to_token[token].score;
}

llama_token_type llama_token_get_type(const struct llama_model* model,
                                      llama_token token) {
...
int llama_tokenize(const struct llama_model* model, const char* text,
                   int text_len, llama_token* tokens, int n_max_tokens,
...
int llama_token_to_piece(const struct llama_model* model, llama_token token,
                         char* buf, int length) {
...
void llama_print_timings(struct llama_context* ctx) {
  ...
  LLAMA_LOG_INFO("%s:        load time = %10.2f ms\n", __func__,
                 timings.t_load_ms);
  LLAMA_LOG_INFO(
      "%s:      sample time = %10.2f ms / %5d runs   (%8.2f ms per token, "
  ...
  LLAMA_LOG_INFO(
      "%s: prompt eval time = %10.2f ms / %5d tokens (%8.2f ms per token, "
      "%8.2f tokens per second)\n",
  ...
  LLAMA_LOG_INFO(
      "%s:        eval time = %10.2f ms / %5d runs   (%8.2f ms per token, "
      "%8.2f tokens per second)\n",
