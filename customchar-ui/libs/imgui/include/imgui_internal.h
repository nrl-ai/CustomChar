...
#define IMGUI_DEBUG_PRINT(_FMT, ...) \
  printf("[%05d] " _FMT, g.FrameCount, __VA_ARGS__)
#endif
...
enum ImGuiButtonFlagsPrivate_ {
  ...
  ImGuiButtonFlags_AllowItemOverlap =
      1
      << 12,  // require previous frame HoveredId to either match id or be null
...
struct ImGuiPopupData {
  ...
  ImGuiWindow* SourceWindow;  // Set on OpenPopup() copy of NavWindow at the
                              // time of opening the popup
...
enum ImGuiNavMoveFlags_ {
  ...
  ImGuiNavMoveFlags_DontSetNavHighlight =
      1 << 12  // Do not alter the visible state of keyboard vs mouse nav
               // highlight
};
...
struct ImGuiContext {
  ...
  ImBitArrayForNamedKeys
      ActiveIdUsingKeyInputMask;  // Active widget will want to read those key
                                  // inputs. When we grow the ImGuiKey enum
                                  // we'll need to either to order the enum to
                                  // make useful keys come first, either
...
struct IMGUI_API ImGuiWindowTempData {
  ...
  ImVec2 CursorStartPosLossyness;  // Record the loss of precision of
                                   ...
                                   // to compensentate and fix the most common
                                   // use case of large scroll area.
